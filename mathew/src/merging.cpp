...
sensor_msgs::LaserScanPtr pointcloud_to_laserscan(sensor_msgs::PointCloud2 *merged_cloud)
{
    ...
    output->header.frame_id = "Robot_base_link";
...
void concat_with_pc()
{
    while (ros::ok())
    {
        ...
        sharedPtr = ros::topic::waitForMessage<sensor_msgs::PointCloud2>("/front_velodyne_points2", ros::Duration(1));
        if (sharedPtr == NULL)
        ...
        sharedPtr = ros::topic::waitForMessage<sensor_msgs::PointCloud2>("/back_velodyne_points2", ros::Duration(1));
        if (sharedPtr == NULL)
            ROS_INFO("No laser messages received");
        else
            cloud_2 = *sharedPtr;
        ...
        concatenated_cloud.header.frame_id = "Robot_base_link";
